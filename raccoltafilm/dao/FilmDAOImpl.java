...
public class FilmDAOImpl implements FilmDAO {
	...
	public Optional<Film> findOne(Long id) throws Exception {
		Film filmResult = entityManager.find(Film.class, id);
	...
	public List<Film> findAllByRegista(Regista input) throws Exception {
		if (input == null) {
			throw new Exception("Problema valore in input");
		}
		
		TypedQuery<Film> query = entityManager.createQuery("select f from Film f where f.regista = ?1", Film.class);
